...
class BlipVQA:
    def __init__(self, core, vision_model_path, text_encoder_path, text_decoder_path, device="CPU"):
        self.processor = BlipProcessor.from_pretrained("Salesforce/blip-vqa-base")
        self.model = BlipForQuestionAnswering.from_pretrained("Salesforce/blip-vqa-base")

        self.vision_model = self.model.vision_model
        self.text_encoder = self.model.text_encoder
        self.text_decoder = self.model.text_decoder

        self.VISION_MODEL_OV = Path(vision_model_path)
        self.TEXT_ENCODER_OV = Path(text_encoder_path)
        self.TEXT_DECODER_OV = Path(text_decoder_path)

        self.core = core
        self.device = device

        self.ov_vision_model = self.core.compile_model(self.VISION_MODEL_OV, self.device)
        self.ov_text_encoder = self.core.compile_model(self.TEXT_ENCODER_OV, self.device)
        self.ov_text_decoder_with_past = self.core.compile_model(self.TEXT_DECODER_OV, self.device)

        self.text_decoder.forward = partial(text_decoder_forward, ov_text_decoder_with_past=self.ov_text_decoder_with_past)

        self.ov_model = OVBlipModel(self.model.config, self.model.decoder_start_token_id, self.ov_vision_model, self.ov_text_encoder, self.text_decoder)

    def preprocess(self, image_path, question):
        raw_image = Image.open(image_path).convert("RGB")
        inputs = self.processor(raw_image, question, return_tensors="pt")
        return raw_image, inputs

    def generate_answer(self, image_path, question, max_length=20):
        raw_image, inputs = self.preprocess(image_path, question)
        start = time.perf_counter()
        out = self.ov_model.generate_answer(**inputs, max_length=max_length)
